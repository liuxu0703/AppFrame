 ...
 */
package lx.af.view.ScrollDetectors;
...
import lx.af.view.ViewFlow.ViewFlow;
...
public class ScrollDetectors {
	...
	private static ScrollDetector getImplements(View v) {
		...
		} else if (v instanceof ViewFlow) {
			imple = new ViewFlowScrollDetector();
	...
	private static class ViewFlowScrollDetector implements ScrollDetector{

		@Override
		public boolean canScrollHorizontal(View v, int direction) {
			// TODO Auto-generated method stub
			
			ViewFlow viewFlow = (ViewFlow) v;
			Adapter adapter = viewFlow.getAdapter();
			if(adapter == null){
				return false;
			}
			
			int currentPostion = viewFlow.getSelectedItemPosition();
			return ((direction < 0 && currentPostion < viewFlow.getChildCount() -1 )
					|| (direction > 0 && currentPostion > 0));
		}
