...
final class M3uConstants {
    ...
    static class Patterns {
        ...
        final static Pattern EXT_X_KEY = Pattern.compile(tagPattern(
                M3uConstants.EXT_X_KEY) + "METHOD=([0-9A-Za-z\\-]*)(,URI=\"(([^\\\\\"]*.*))\")?");
        final static Pattern EXT_X_TARGET_DURATION = Pattern.compile(
                tagPattern(M3uConstants.EXT_X_TARGET_DURATION) + "([0-9]*)");
        final static Pattern EXT_X_MEDIA_SEQUENCE = Pattern.compile(
                tagPattern(M3uConstants.EXT_X_MEDIA_SEQUENCE) + "([0-9]*)");
        final static Pattern EXT_X_PROGRAM_DATE_TIME = Pattern.compile(
                tagPattern(M3uConstants.EXT_X_PROGRAM_DATE_TIME) + "(.*)");

        final static Pattern EXT_X_STREAM_INF_PROGRAM_ID = Pattern.compile(
                "(?::|\\s*)PROGRAM-ID=(\\d*)(?:,.+|\\s)?");
        final static Pattern EXT_X_STREAM_INF_BANDWIDTH = Pattern.compile(
                "(?::|\\s*)BANDWIDTH=(\\d*)(?:,.+|\\s)?");
        final static Pattern EXT_X_STREAM_INF_CODECS = Pattern.compile(
                "(?::|\\s*)CODECS=\"(.*)\"(?:,.+|\\s)?");
